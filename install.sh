...
describe_component() {
    ...
    case "$1" in
        ...
        zsh)          echo '~/.zshrc, ~/.config/zsh/ (fallback shell; env comes from mkenv)' ;;
...
# The zsh side of the same setup: ~/.zshrc plus ~/.config/zsh/. The
# machine-specific ~/.config/zsh/env is generated by mkenv, which the bash
# component runs -- install this on its own and the init script simply skips
# a missing env file until `~/.config/bash/mkenv' is next run.
install_zsh() {
    copy_file ".zshrc" "$HOME/.zshrc"
    copy_tree "$DOTCONFIG_DIR/zsh" "$XDG_CONFIG_DIR/zsh"
}
